...
void types_write_entries(Sqlpilot *sqlpilot, const gchar *id)
{
  const gchar
    *ident,
    *make,
  ...
  prop        = gtk_toggle_button_get_active(GTK_TOGGLE_BUTTON(sqlpilot->types_prop));
  ...
  db_bind_int(stmt, TYPES_WRITE_PROP, prop ? 1 : 0);
...
void types_load_entries_from_selection(Sqlpilot *logb)
{
  GtkTreeIter iter;
  GtkTreeModel *tmodel;
  ...
  if (gtk_tree_selection_get_selected (logb->types_selection, &tmodel, &iter)) {
    ...
    gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(logb->types_prop), str_bool(prop));
